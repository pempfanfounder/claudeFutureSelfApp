...
const program = ts.createProgram(
  [
    virtual,
    path.join(root, "push-dispatch/index.ts"),
    path.join(root, "push-receipts/index.ts"),
  ],
  options,
  host,
);
...
if (diagnostics.length) {
  ...
  console.log(
    "Push Edge TypeScript: no diagnostics (external runtime declarations mocked; no Deno/provider/DB integration).",
  );
